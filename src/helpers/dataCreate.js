...
async function dataCreate() {
  //   await mongoose.connection.dropDatabase(); Böyle de tüm collectionları silerek de silme işlemi yapılabilirdi. 
  await Department.deleteMany();
  await Department.insertMany(departments);
  await Personnel.deleteMany();
  // bu func controllerda yaoptığımız gibi depertman boş ise ilk kullanıcıya admin özelliği veriyor.
  let isFirstAccount = (await Personnel.countDocuments()) === 0;
  //   await Personnel.insertMany(personnels); bu yorumda olunca çalışıyor
