...
module.exports = {
  list: async (req, res) => {
    /*
        ...
        `
    */
    const data = await res.getModelList(Department);//sortFindSearch a göre yazdığım func şeklinde listeleyecek. 

    res.status(200).send({
      error: false,
      data,
      detail: await res.getModelListDetails(Department),//quaryHandler da oluşturulan func datamın detaillerini verecek.(MODELADI OLACAK)
    });
  },
  ...
  //?update in alacağı parametreler: filter, update,options
  ...
  update: async (req, res) => {
    ...
    */
    ...
    res.status(202).send({
      error: false,
      data,
      new: await Department.findOne({ _id: req.params.id }),// güncellenen veriyi döner
    });
  },
  ...
  delete: async (req, res) => {
    ...
    */
    ...
    res.status(data.deletedCount ? 204 : 404).send({
      ...
      data,
    });//deletedCount DATANIN ŞÇİNDE OLAN BİR KEY
  },
//? departman İd si gelince o departmana ait personelleri listeleyecek kontroller yazıldı.
  personnels: async (req, res) => {
    ...
    */
    ...
    const data = await res.getModelList(// listelemeyi quaryHandler middlerware e göre yapılıyor.
      Personnel,
