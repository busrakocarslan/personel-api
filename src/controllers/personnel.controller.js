...
module.exports = {
  ...
  create: async (req, res) => {
    ...
    */
   //? ilk kullanıcıyı admin olarak ekleme function i
    // const isFirstAccount = (await Personnel.countDocuments()) === 0;
    // req.body.isAdmin = isFirstAccount ? true : false;

    // isLead Control:
    // her depertmanın bir leadı olması gerketiğinden lead kontrolü yapılıyor. 
    const isLead = req.body?.isLead || false;
    if (isLead) {//islead=true ise bu kısma giriyor
      const xyz = await Personnel.updateMany(// burası bir çok datayı güncellediği için updateMany ile yapılıyor. 
        { departmentId: req.body.departmentId, isLead: true },// bu kısım filtreleme yapıyor.  bu departmana ait lead olanları getirip false yapıyor.
        { isLead: false },//güncellenecek alan, true olanları false çeviriyor. 
      );
    }
  ...
  login: async (req, res) => {
    ...
    if (username && password) {
      ...
      if (user && user.isActive) {
        // Set Session:
        req.session = {//* indexte cookie ye name yazmazsak adı default session oluyor
          id: user._id,
